...
def main():
    ...
    # Save path
    save_path = "./new_Fed_sass_v3_result_1_clients"
    ...
    # Compute and store data distribution
    ...
    for round_idx in range(num_global_rounds):
        ...
        for client_idx in range(num_clients):
            ...
            # 6) Check final LR used by this client
    ...
    plt.figure(figsize=(8, 6))
    plt.plot(range(1, num_global_rounds + 1), accuracy_over_rounds, 'o-g', label="Global Accuracy")
    plt.xlabel("Global Round")
    ...
    # (4) Plot accuracy per client and global accuracy vs global rounds
    ...
    for client_idx in range(num_clients):
        plt.plot(
            ...
            marker='o',
            linestyle='-',
            label=f"Client {client_idx} Accuracy"
        )
    plt.plot(
        range(1, num_global_rounds+1),
        accuracy_over_rounds,
        ...
        label="Server (Global) Accuracy"
    )
    plt.xlabel("Global Round")
    plt.ylabel("Accuracy (%)")
    plt.title("Client and Server Accuracy vs. Global Round")
    plt.legend()
    plt.grid(True)
    plt.savefig(os.path.join(save_path, "client_and_server_accuracy.png"))
    plt.close()

    # (5) Plot client data distribution
    plt.figure(figsize=(10, 6))
    ind = np.arange(num_clients)
    width = 0.08
    for cls in range(num_classes):
        counts = [client_data_distribution[client_idx][cls] for client_idx in range(num_clients)]
        plt.bar(ind + cls*width, counts, width, label=f"Class {cls}")
    plt.xlabel("Client")
    plt.ylabel("Number of Samples")
    plt.title("Client Data Distribution per Class")
