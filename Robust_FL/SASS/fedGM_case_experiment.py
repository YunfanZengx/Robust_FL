...
# 2) try use CIFAR-10
# python /home/local/ASURITE/yzeng88/fedSASS/SASS/fedGM_case_experiment.py --num_clients 5 --global_rounds 10 --local_epochs 1 --dist case1
# python /home/local/ASURITE/yzeng88/fedSASS/SASS/test2_extreme_FedGM.py --num_clients 10 --global_rounds 100 --local_epochs 1 --dist extreme
...
def main():
    ...
    train_loss_vs_passes = [[] for _ in range(args.num_clients)]  # list of lists of (nn_pass, train_loss)
    test_loss_vs_passes  = [[] for _ in range(args.num_clients)]  # list of lists of (nn_pass, test_loss)
    ...
    with open(result_path, "w") as f, open(server_acc_path, "w") as f_server:
        ...
        for rnd in range(args.global_rounds):
            ...
            for cid in range(args.num_clients):
                ...
                # Link the SASS optimizer to this client's model
    ...
    if args.num_clients == 1:
        # If only 1 client, 'axes' is not a list; make it a list for uniformity
        axes = [axes]
    for cid in range(args.num_clients):
    ...
    for cid in range(args.num_clients):
        if len(test_loss_vs_passes[cid]) > 0:
            ...
            axes[cid].plot(passes, losses, color='red', linewidth=1)
