...
# python /home/local/ASURITE/yzeng88/fedSASS/SASS/FedGM_SASS_experiment.py --num_clients 1 --num_global_rounds 50 --local_epochs 1 --partition dirichlet --alpha 0.2
# python /home/local/ASURITE/yzeng88/fedSASS/SASS/FedGM_SASS_experiment.py --num_clients 5 --num_global_rounds 50 --local_epochs 2 --partition dirichlet --alpha 0.2
# python /home/local/ASURITE/yzeng88/fedSASS/SASS/FedGM_SASS_experiment.py --num_clients 5 --num_global_rounds 50 --local_epochs 2 --partition iid
...
def geometric_median_weiszfeld(vectors, max_iter=100, tol=1e-4):
    """
    vectors: list of T tensors of the same shape (the points).
    We find x that minimizes sum_i ||x - vectors[i]||2.
    Implementation of Weiszfeld's algorithm.

    Returns the geometric median as a tensor with the same shape.
    """
    # Start x at the arithmetic mean (or any guess)
    x = sum(vectors) / len(vectors)

    for _ in range(max_iter):
        x_old = x.clone()
        # distances from x to each vector
        dist = torch.stack([(x - v).norm(p=2) for v in vectors])
        # If any distance is extremely small, that point is the solution
        min_dist = dist.min()
        if min_dist < 1e-12:
            # x is effectively one of the vectors => done
            x = vectors[dist.argmin()]
            break

        # Weiszfeld update:
        weights = 1.0 / torch.clamp(dist, min=1e-12)
        ...
        # Check convergence
        if (x - x_old).norm(p=2) < tol:
            break

    return x

def FedGeometricMedian(list_of_models, max_iter=20, tol=1e-4):
    ...
    for key in base_params.keys():
        # Collect all the client parameters for this key
        client_tensors = []
        for m in list_of_models:
            client_tensors.append(m.state_dict()[key])

        client_tensors = [ct.to(base_params[key].device) for ct in client_tensors]

        # Use Weiszfeld's algorithm to compute geometric median for this parameter
        gm = geometric_median_weiszfeld(client_tensors, max_iter=max_iter, tol=tol)
        aggregated_state[key] = gm

    return aggregated_state

# 5) SIMPLE DISTRIBUTION PLOT
...
# 6) MAIN 
def main():
    parser = argparse.ArgumentParser(description="Federated Geometric Median w/ SASS + Weiszfeld.")
    parser.add_argument('--num_clients', type=int, default=5, help="Number of clients")
    parser.add_argument('--num_global_rounds', type=int, default=5, help="Number of global rounds")
    ...
    base_folder = "./FedGM_SASS_results"
    os.makedirs(base_folder, exist_ok=True)
    exp_name = f"FedGMedian_{args.num_clients}clients_{args.num_global_rounds}global_{args.local_epochs}local_{args.partition}"
    if args.partition == 'dirichlet':
    ...
    for round_idx in range(num_global_rounds):
        ...
        new_weights = FedGeometricMedian(local_models, max_iter=20, tol=1e-4)
        ...
        print(f"Round {round_idx+1} - Global GeomMedian Accuracy: {test_acc:.2f}%")
    ...
    plt.ylabel("Accuracy (%)")
    plt.title("Accuracy: Clients vs. Global (Geometric Median)")
    plt.legend()
    plt.grid(True)
    ...
    plt.title("Learning Rate per Client (Linear Scale) - GeomMedian")
    plt.legend()
    plt.grid(True)
    ...
    plt.title("Learning Rate per Client (Log Scale) - GeomMedian")
    plt.legend()
    ...
    plt.title("Per-Client Training Loss vs. Round - GeomMedian")
    plt.legend()
    plt.grid(True)
    ...
    print("\nAll done! Results (Geometric Median) in:", save_path)
